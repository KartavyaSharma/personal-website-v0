            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-8'>
                {
                    props.listData.map(project => {
                        ...
                        return (
                            <div className='group'>
                                <div className='absolute z-50 border text-opacity-80 border-border text-white font-mono font-semibold text-base md:text-lg bg-background px-2 py-1 -mt-4 rounded-t-lg rounded-r-lg shadow-lg'>
                                    {project.node.category}
                                </div>
                                <div className='relative z-10 border h-full border-border w-full rounded md:group-hover:bg-opacity-10 shadow-lg'>
                                    <div className='grid grid-rows-3 h-full p-4 md:p-8 md:pb-0 md:pt-5'>
                                        ...
                                        <div className='flex flex-row items-center h-full'>
                                            <Link to={`/project/${project.node.name.replace(' -', '').split(' ').join('-').toLowerCase()}`}>
                                                <button className='rounded px-2 py-1 hover:bg-hover-bg outline-none border border-border border-opacity-50 text-white text-opacity-80 font-mono mr-3'>
                                                    Details
                                                </button>
                                            </Link>
                                            {
                                                project.node.projectlink !== "" ? (
                                                    <a href={`${project.node.projectlink}`} target='_blank' rel='noreferrer'>
                                                        <button className='py-2 px-4 mr-3 text-white rounded outline-none border border-opacity-50 border-border hover:bg-hover-bg flex'>
                                            ...
                                            }
                                            {
                                                project.node.githublink !== "" ? (
                                                    <a href={`${project.node.githublink}`} target='_blank' rel='noreferrer'>
                                                        <button className='py-2 px-4 text-white rounded outline-none border border-opacity-50 border-border hover:bg-hover-bg flex'>
